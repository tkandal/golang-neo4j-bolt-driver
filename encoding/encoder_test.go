...
func TestEncodeNil(t *testing.T) {
    ...
    if err = binary.Write(expectedBuf, binary.BigEndian, uint16(1)); err != nil {
        t.Fatalf("binary write failed; error = %v\n", err)
    }
...
func generateIntExpectedBuf(val int64) ([]byte, error) {
    ...
    case val >= math.MinInt64 && val < math.MinInt32:
        _ = binary.Write(expectedBuf, binary.BigEndian, uint16(9))
        expectedBuf.Write([]byte{Int64Marker})
        _ = binary.Write(expectedBuf, binary.BigEndian, int64(val))
    case val >= math.MinInt32 && val < math.MinInt16:
        _ = binary.Write(expectedBuf, binary.BigEndian, uint16(5))
        expectedBuf.Write([]byte{Int32Marker})
        _ = binary.Write(expectedBuf, binary.BigEndian, int32(val))
    case val >= math.MinInt16 && val < math.MinInt8:
        _ = binary.Write(expectedBuf, binary.BigEndian, uint16(3))
        expectedBuf.Write([]byte{Int16Marker})
        _ = binary.Write(expectedBuf, binary.BigEndian, int16(val))
    case val >= math.MinInt8 && val < -16:
        _ = binary.Write(expectedBuf, binary.BigEndian, uint16(2))
        expectedBuf.Write([]byte{Int8Marker})
        _ = binary.Write(expectedBuf, binary.BigEndian, int8(val))
    case val >= -16 && val <= math.MaxInt8:
        _ = binary.Write(expectedBuf, binary.BigEndian, uint16(1))
        _ = binary.Write(expectedBuf, binary.BigEndian, int8(val))
...
func TestEncodeFloat32(t *testing.T) {
    expected := func(val float32) []byte {
        ...
        _ = binary.Write(expectedBuf, binary.BigEndian, float64(val))
        expectedBuf.Write(EndMessage)
...
func TestEncodeString(t *testing.T) {
    expected := func(val string) []byte {
        ...
        case length <= 15:
            ...
            expectedBuf.Write(expBytes)
        case length > 15 && length <= math.MaxUint8:
            expectedBuf.Write([]byte{String8Marker})
            _ = binary.Write(expectedBuf, binary.BigEndian, int8(length))
            expectedBuf.Write(expBytes)
        case length > math.MaxUint8 && length <= math.MaxUint16:
            expectedBuf.Write([]byte{String16Marker})
            _ = binary.Write(expectedBuf, binary.BigEndian, int16(length))
            expectedBuf.Write(expBytes)
        case length > math.MaxUint16 && int64(length) <= math.MaxUint32:
            expectedBuf.Write([]byte{String32Marker})
            _ = binary.Write(expectedBuf, binary.BigEndian, int32(length))
            expectedBuf.Write(expBytes)
        default:
            t.Fatalf("String too long to write: %s", val)
        }

        if err := binary.Write(resultExpectedBuf, binary.BigEndian, uint16(expectedBuf.Len())); err != nil {
            t.Fatal(err)
        }
        _, _ = resultExpectedBuf.ReadFrom(expectedBuf)
