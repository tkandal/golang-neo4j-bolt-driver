...
func (d Decoder) readData(messageLen uint16) ([]byte, error) {
	...
	totalRead := uint16(0)
	// readBuf := make([]byte, messageLen)

	for totalRead < messageLen {
		numRead, err := d.r.Read(output[totalRead:])
		// numRead, err := output.ReadFrom(d.r)
		...
		// output = append(output, readBuf[:numRead]...)
...
func (d Decoder) Decode() (interface{}, error) {
	output := buffer.Pool.Get().(*bytes.Buffer)
	output.Reset()
	defer buffer.Pool.Put(output)

	data, err := d.read(output)
	if err != nil {
		return nil, err
	}
...
func (d Decoder) decode(buffer *bytes.Buffer) (interface{}, error) {
	...
	// if it's a TINYINT
	var markerInt = int8(marker)

	// ** Makes no sense to read a 8-bit int with binary, only when an int is bigger than one byte
