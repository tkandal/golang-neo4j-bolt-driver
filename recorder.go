...
func (r *recorder) Read(b []byte) (n int, err error) {
    ...
    if event.IsWrite {
        return 0, errors.New("recorder expected read, but got write; %#v; event %#v", r, event)
    }
...
func (r *recorder) Write(b []byte) (n int, err error) {
    ...
    event := r.events[r.currentEvent]
    if !event.IsWrite {
        return 0, errors.New("recorder expected write, but got read; %#v; event %#v", r, event)
    }
...
func (r *recorder) print() {
    ...
    for _, event := range r.events {
        ...
        fmt.Printf("%s @ %d", typee, event.Timestamp)
        ...
        if event.Error != nil {
            fmt.Printf("error occured during event; error = %s\n", event.Error)
        }
        fmt.Printf("\n\n")
    }

    fmt.Printf("recording end %s\n", r.name)
}
